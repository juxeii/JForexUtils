...
public class BarQuoteEngineTest extends CurrencyUtilForTest {

    private BarQuoteProvider barQuoteEngine;

    @Mock private BarQuoteConsumer barQuoteConsumerEURUSDMock;
    @Mock private BarQuoteConsumer barQuoteConsumerAUDUSDMock;
    @Mock private IBar askBarEURUSDOfHistory;
    @Mock private IBar bidBarEURUSDOfHistory;
    @Mock private IBar firstAskBarEURUSD;
    @Mock private IBar firstBidBarEURUSD;
    @Mock private IBar secondAskBarEURUSD;
    @Mock private IBar secondBidBarEURUSD;
    @Mock private IBar firstAskBarAUDUSD;
    @Mock private IBar firstBidBarAUDUSD;
    ...
    public void setUp() {
        ...
        barQuoteEngine = new BarQuoteProvider(barObservable, historyMock);
        barQuoteEngine.subscribe(instrumentEURUSD, testPeriod, barQuoteConsumerEURUSDMock::onBarQuote);
    }

    private void verifyBarValues(final IBar askBar,
                                 final IBar bidBar) {
        assertThat(barQuoteEngine.askBar(instrumentEURUSD, testPeriod),
                   equalTo(askBar));

        assertThat(barQuoteEngine.bidBar(instrumentEURUSD, testPeriod),
                   equalTo(bidBar));

        assertThat(barQuoteEngine.forOfferSide(instrumentEURUSD, testPeriod, OfferSide.ASK),
                   equalTo(askBar));

        assertThat(barQuoteEngine.forOfferSide(instrumentEURUSD, testPeriod, OfferSide.BID),
                   equalTo(bidBar));
    }

    @Test
    public void testObservableIsCorrectReturned() {
        assertThat(barQuoteEngine.observable(), equalTo(barObservable));
    }
    ...
    public class HistoryThrowsBeforeFirstBarQuoteIsReceived {
        ...
        public void setUp() throws JFException {
            when(historyMock.getBar(eq(instrumentEURUSD), eq(testPeriod), any(), eq(1))).thenThrow(jfException);
        }

        @Test(expected = QuoteProviderException.class)
        public void testAskBarThrows() {
            barQuoteEngine.askBar(instrumentEURUSD, testPeriod);
        }

        @Test(expected = QuoteProviderException.class)
        public void testBidBarThrows() {
            barQuoteEngine.bidBar(instrumentEURUSD, testPeriod);
        }

        @Test(expected = QuoteProviderException.class)
        public void testForOfferSideAskThrows() {
            barQuoteEngine.forOfferSide(instrumentEURUSD, testPeriod, OfferSide.ASK);
        }

        @Test(expected = QuoteProviderException.class)
        public void testForOfferSideBidThrows() {
            barQuoteEngine.forOfferSide(instrumentEURUSD, testPeriod, OfferSide.BID);
        }
    }
    ...
    public class AfterFirstBarQuoteIsConsumed {
        ...
        public class AfterRegisterDifferentPeriod {
            ...
            @Before
            public void setUp() {
                barQuoteEngine.subscribe(instrumentEURUSD, otherPeriod, barQuoteConsumerEURUSDMock::onBarQuote);
                firstEURUSDBarQuote =
        ...
        public class AfterSecondBarQuoteIsConsumed {
            ...
            public void testBarQuoteConsumerForAUDUSDReceivesBarQuote() {
                ...
                barQuoteEngine.subscribe(instrumentAUDUSD, testPeriod, barQuoteConsumerAUDUSDMock::onBarQuote);
